...
export function StatCards({ stats }: Props) {
  ...
  const totalCacheRead = stats.total_cache_read_tokens;
  const totalCacheCreation = stats.total_cache_creation_tokens;

  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-(--spacing-card-gap)">
      <StatCard label="Input vs Output" subtitle={`${formatTokens(totalCacheRead)} cached read · ${formatTokens(totalCacheCreation)} cached write`}>
